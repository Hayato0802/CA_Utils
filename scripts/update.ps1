...
if (Test-Path "$installDir\manifest.json") {
    ...
    if ($currentVersion -eq $latestVersion) {
        Write-Host "ä˘Ç…ç≈êVî≈Ç™DLÇ≥ÇÍÇƒÇ¢Ç‹Ç∑ (v$currentVersion)" -ForegroundColor Green
        Start-Process (Resolve-Path $installDir)
        exit 1
    }
    Write-Host "çXêVíÜ... from v$currentVersion to v$latestVersion" -ForegroundColor Cyan
    Start-Process (Resolve-Path $installDir)
} else {
